...
export const createTask = async (taskData, projectId) => {
  ...
  const data = {
    ...
    sequentialSubtasks: taskData?.sequentialSubtasks,
...
export const markSentToClient = async (itemId, payload = {}) => {
  const { data } = await apiClient.post(
    `/tasks/${itemId}/sent-to-client`,
    payload
  );
  return data;
};
...
export const closeEmployeeProbation = async (employeeId, payload) => {
  const { data } = await apiClient.patch(
    `/employee/${employeeId}/probation/end`,
    payload
  );
  return data;
};
