...
const AddSubTask = ({
  ...
  const { data: socialMediaData } = useGetProjectSocialMedia(
    projectId && (!projectData?.socialMedia || Object.keys(projectData.socialMedia).length === 0) ? projectId : null
  );
  console.log(socialMediaData)

  // Use projectData if available, otherwise use fetched social media data
  const effectiveProjectData = projectData?.socialMedia ? projectData : { socialMedia: socialMediaData?.socialMedia || {} };
                ...
                {values.title === "publish" && (
                  <div className="space-y-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-medium text-gray-700">Publish URLs</span>
                    </div>
                    ...
                    {effectiveProjectData?.socialMedia?.other?.map((platform, index) => (
                      platform.manage && (
