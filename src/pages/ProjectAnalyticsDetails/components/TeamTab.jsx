    ...
    <div className="space-y-6">
          ...
          <ul className="divide-y divide-gray-200">
            {project?.teams?.map((member) => (
              <li
                ...
                key={member._id}
                className="px-4 py-3 sm:px-6 cursor-pointer hover:bg-gray-50 transition-colors"
              >
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        <div className="px-4 py-4 sm:px-6">
          ...
          <p className="mt-1 max-w-2xl text-xs text-gray-500">
            People who are reporters/monitors for this project.
          </p>
        </div>
        <div className="border-t border-gray-200">
          <ul className="divide-y divide-gray-200">
            {project?.reporters?.map((member) => (
              <li
                onClick={() => navigate(`/employees/${member._id}`)}
                key={member._id}
                className="px-4 py-3 sm:px-6 cursor-pointer hover:bg-gray-50 transition-colors"
            ...
            {(!project?.reporters || project.reporters.length === 0) && (
              <li className="px-4 py-3 sm:px-6 text-center text-xs text-gray-500">
                No reporters assigned to this project.
              </li>
            )}
          </ul>
        </div>
