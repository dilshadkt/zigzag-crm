        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <input
              ...
              onChange={(event) => {
                ...
                if (!isMandatory || 
                    (field.key === "system_name" && event.target.value.toLowerCase().includes("name")) ||
                    (field.key === "system_email" && (event.target.value.toLowerCase().includes("email") || field.type === "email")) ||
                    (field.key === "system_phone" && (event.target.value.toLowerCase().includes("contact") || event.target.value.toLowerCase().includes("phone") || field.type === "tel"))) {
          ...
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <button
              ...
              onClick={() => {
                if (isMandatory) {
                  alert("This is a mandatory field and cannot be deleted. Full Name, Email, and Contact are required for the lead form to work properly.");
                  return;
                }
                if (
                  window.confirm(
