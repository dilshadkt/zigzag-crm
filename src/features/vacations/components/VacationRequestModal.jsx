...
const VacationRequestModal = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.isHalfDay && !formData.halfDayType) {
      setSubmitError(
        "Please select which half of the day you are requesting."
      );
      setIsSubmitting(false);
      return;
    }
  ...
  const selectDate = (date) => {
    if (selectionMode === "start") {
      ...
      if (isBefore(date, startDate)) {
        ...
        const isSingle = isSameDay(date, startDate);
        setFormData((prev) => ({
          ...prev,
          endDate: format(date, "yyyy-MM-dd"),
          isHalfDay: isSingle ? prev.isHalfDay : false,
          halfDayType: isSingle ? prev.halfDayType : "",
        }));
      }
  ...
  const durationDays = Math.ceil(Math.abs(new Date(formData.endDate) - new Date(formData.startDate)) / (1000 * 60 * 60 * 24)) + 1;
  const duration = formData.isHalfDay ? 0.5 : durationDays;
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setFormData(prev => ({
                          ...
                          halfDayType: e.target.checked ? "first_half" : "",
                        }));
                      }}
                      className="w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
                    />
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Request Half Day</span>
                  </label>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className={`flex-1 py-2 rounded-xl text-[10px] font-bold transition-all ${
                          formData.halfDayType === 'first_half'
                      ...
                      <button
                        ...
                        className={`flex-1 py-2 rounded-xl text-[10px] font-bold transition-all ${
                          formData.halfDayType === 'second_half'
                            ? "bg-indigo-50 text-indigo-700 border-indigo-200"
                            : "bg-slate-50 text-slate-500 hover:bg-slate-100"
                        } border`}
                      >
