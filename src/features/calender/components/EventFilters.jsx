...
const EventFilters = ({
  ...
  const getSelectedProjectName = () => {
    if (!projectFilter || projectFilter.length === 0) return "All Projects";
    if (projectFilter.length === 1) {
      const selectedProject = projects.find((p) => p.id === projectFilter[0]);
          ...
          <div className="absolute top-full left-0 mt-1 w-48 bg-white border border-gray-200 rounded-md shadow-lg z-50 max-h-60 overflow-y-auto">
            <div className="py-1">
              ...
              >
                <input
                  type="checkbox"
                  checked={!projectFilter || projectFilter.length === 0}
                  readOnly
                  className="w-3 h-3 rounded text-blue-600 focus:ring-blue-500"
                />
              ...
              {projects.length > 0 ? (
                projects.map((project) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={projectFilter && projectFilter.includes(project.id)}
                      readOnly
                      className="w-3 h-3 rounded text-blue-600 focus:ring-blue-500"
                    />
                    <div
